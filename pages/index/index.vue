	...
	<view class="page">
		<uni-nav-bar status-bar="true" :border="false">
			...
			<view class="search-all-wrap">
				<view class="search-wrap" @click="search">
					...
					<input disabled="true" class="uni-input" placeholder="搜索课程, 老师"
		...
		<scroll-view class="index-all-wrap" scroll-y="true" :style="'height:' + scrollH+'rpx;'">
			...
			<view class="index-banner-wrap">
				<swiper class="index-banner swiper" circular="false" :indicator-dots="indicatorDots"
					...
					<swiper-item @click="bannerTo(item)" v-for="(item, index) in bannerList" :key="item.id">
						<image class="index-banner-img"
							src="https://demo.sdwanyue.com/upload/admin/20220905/d21c4b1e38739f731f32f2669f2d1eb6.png"
			...
			<view class="index-course-wrap">
				<view @click="getCourseByClass(item.id, item.name)" class="know-item course-item scroll-view-item_H"
					v-for="(item, index) in course_class" :key="item.id">
			...
			<view class="course-list-wrap">
				...
				<view class="hot-all-wrap">
					...
					<view class="hot-img-wrap">
						<block v-if="hotlist != undefined">
							<image v-if="hotlist.length > 0" class="hot-left-img" :src="hotlist[0].thumb"
								mode="aspectFill" @click="viewContentInfo(hotlist[0].id,hotlist[0].paytype)"></image>
							<image v-if="hotlist.length >= 2" class="hot-right-top-img" :src="hotlist[1].thumb"
								mode="aspectFill" @click="viewContentInfo(hotlist[1].id,hotlist[1].paytype)"></image>
							<image v-if="hotlist.length >= 3" class="hot-right-bottom-img" :src="hotlist[2].thumb"
								mode="aspectFill" @click="viewContentInfo(hotlist[2].id,hotlist[2].paytype)"></image>
						</block>
					</view>
				...
				<view class="course-list-wrap">
					<view class="live-title-wrap live-ketang-title-wrap">
						<!-- 标题 更多 -->
						<view class="icon-title-wrap">
							<image class="hot-title-img" mode="aspectFit" src="../../static/images/lve_ketang_icon.png">
							</image>
					...
					<view class="live-ketang-wrap">
						<scroll-view class="scroll-view_H" scroll-x="true" @scroll="scroll" scroll-left="120">
							<view @click="viewLiveInfo(item.id,item.paytype)"
								class="live-ketang-item scroll-view-item_H" v-for="(item, index) in live_info"
								:key="item.id">
								...
								<view class="live-teacher-price live-ketang-t-price">
									<image class="live-ketang-teacher-avatar" :src="item.avatar" mode="aspectFill">
									</image>
				...
				<view class="course-list-wrap content-list-wrap">
					...
					<view class="la-wrap">
						<view class="live-title-wrap live-content-title-wrap">
							<!-- 标题 更多 -->
							<view class="icon-title-wrap">
								<image class="hot-title-img" mode="aspectFit"
									src="../../static/images/jingxuan_content.png"></image>
								<text class="live-title live-content-title">精选内容</text>
							</view>
							<text @click="contentmore" class="live-more content-more-btn">更多&nbsp;&nbsp;<text
									class="iconfont icon-jinrujiantou c-more-btn-icon"></text></text>
						</view>
					</view>

					<view class="course-wrap">
						<view @click="viewContentInfo(item.id,item.paytype)" class="live-list live-list-know"
							v-for="(item, index) in list_info" :key="index">
	...
	export default {
		...
		methods: {
			...
			getCourseByClass(courseCid, courseCname) {
				...
				uni.navigateTo({
					url: '../course_class_list/course_class_list?course_cid=' + courseCid + '&course_cname=' +
						courseCname,
				});
			...
			viewContentInfo(contentCourseId, contentCoursetype) {
				uni.navigateTo({
					url: '../../packageB/pages/content-info/content-info?courseid=' + contentCourseId +
						'&paytype=' +
						contentCoursetype,
				});
			},
			...
			viewCourseInfo(contentCourseId, contentCoursetype) {
				uni.navigateTo({
					url: '../../packageB/pages/courseinfo/courseinfo?courseid=' + contentCourseId + '&paytype=' +
						contentCoursetype,
