<template>
	<view>
		<view class="live-all-wrap">
			...
			<template v-else-if="livetype == 21">
			...
			<template v-else-if="livetype == 2">
				...
				<player-component id="tvp-id" @timeupdate="handleTimeUpdate" @play="handlePlay" @pause="handlePause"
					@ended="handleEnded" playerid="tvp" :vid="pull">
					<!-- 里面可以放插槽 -->
				</player-component>
	...
	export default {
		...
		data() {
			return {
				...
				stats: {
					lastTime: 0, // 上次记录时间(秒)
					lastReport: 0, // 上次上报时间戳
					interval: 10000, // 上报间隔(ms)
					minProgress: 0.05, // 最小进度变化(5%)
					duration: 0 // 视频总时长
				}
			}
		},
		...
		methods: {
			...
			handleTimeUpdate(e) {
				...
				this.stats.duration = e.detail.duration;

				const now = Date.now();
				const progressChanged = Math.abs(current - this.stats.lastTime) / this.stats.duration;

				// 满足任一条件就上报：时间间隔OR进度变化足够大
				if (now - this.stats.lastReport >= this.stats.interval ||
					progressChanged >= this.stats.minProgress) {
						
					this.stats.lastTime = Math.round(current);
			...
			reportProgress(currentTime) {
				const data = {
					...
					percent: (currentTime / this.stats.duration * 100).toFixed(2),
					timestamp: Date.now()
				};

				console.log('上报进度:', data);
			...
			submitWatchDuration() {
				...
				if (this.stats.lastTime === 0) {
					console.warn('观看时长:', this.stats.lastTime);
					return;
				}
				uni.request({
					...
					data: {
						...
						duration: this.stats.lastTime,
						sharer_id: this.liveInfo.sharer_id,
					},
					success: res => {
						console.log('✅ 观看时长提交成功:', this.stats.lastTime + '秒');
					},
					fail: err => {
				...
				uni.request({
					...
					success: res => {
						...
						if (res.data.data.code == 0) {
							...
							this.showppt_index = this.pptindex + 1;
							if (res.data.data.info && Array.isArray(res.data.data.info) && res.data.data.info[
									0]) {
			...
			onSliderDragStart(e) {
				...
				document.addEventListener('touchend', this.onSliderUp, {
					passive: false
				});
				// #endif
			},
			...
			updateSlider(e, commit = false) {
				...
				console.log('updateSlider:', {
					clientX,
					percent,
					sliderProgress: this.sliderProgress,
